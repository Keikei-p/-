...
- owner 本人は、そのまま自己削除できません。先に別の有効メンバーへ owner を移譲します。
- owner 移譲は会社・旧owner・新ownerの関連ドキュメントを一括更新し、新ownerを leader にします。
- 移譲後の旧ownerは leader として残るため、退職する場合はその後に自分のアカウントを削除できます。
...
- owner移譲時に会社のownerUid、旧owner、新ownerの情報が同じ一括更新で整合していること
- 一般メンバーが他メンバーのメールアドレス一覧を取得できないこと
...
1. 新規登録後にメール確認が必要であること
2. 会社Aのメンバーから会社Bのデータが見えないこと
3. 招待リンクが30分後に利用できないこと
4. 招待先と異なるメールアドレスでは参加できないこと
5. ownerがメンバーを利用停止すると、そのメンバーが会社へアクセスできないこと
6. owner移譲後、新ownerだけがowner操作を行えること
7. 旧ownerが移譲後に自分のアカウントを削除できること

## 将来用 Functions について

`functions/` はBlazeプランへ移行する場合の参考実装として残しています。
現在の `firebase.json` にはFunctionsデプロイ設定を含めていないため、本番のSpark運用では使用されません。
